...
export const calculateStats = (speciesData) => {
  return speciesData.map((data) => {
    const avgConfidence =
      data.totalCount > 0 ? (data.confidenceSum / data.totalCount) * 100 : 0;
    ...
    if (data.locations.length > 0) {
      const avgLat =
        data.locations.reduce((sum, loc) => sum + loc[1], 0) /
        data.locations.length;
      const avgLng =
        data.locations.reduce((sum, loc) => sum + loc[0], 0) /
        data.locations.length;
    ...
    const riskLevel =
      mostRecentSighting?.analysis?.llm?.details?.risk_level || "Medium";
    const scientificName =
      data.llmData?.details?.advisory_content?.species_identification
        ?.scientific_name || "";
...
export const analyzeInvasiveRisk = (invasiveSpecies) => {
  ...
  return {
    ...
    avgSightingsPerSpecies:
      invasiveSpecies.length / Math.max(Object.keys(speciesGroups).length, 1),
  };
};
...
export const identifyHotspots = (sightings) => {
  ...
  activeSightings.forEach((s) => {
    ...
    const key = `${Math.round(lat / tolerance) * tolerance},${
      Math.round(lng / tolerance) * tolerance
    }`;
...
export const createDensityAnalysis = (sightings) => {
  ...
  if (densityValues.length === 0)
    return { grid: {}, maxDensity: 0, hotspots: 0, averageDensity: 0 };
...
export const calculateTrends = (sightings) => {
  ...
  const weeklyChange =
    lastWeek > 0
      ...
      ? 100
      : 0;
  const lastSightingDate =
    sightings.length > 0
      ? new Date(sightings[sightings.length - 1].createdAt)
      : now;
  const daysSinceLastSighting =
    (now - lastSightingDate) / (1000 * 60 * 60 * 24);
...
const getThreatRecommendation = (level, count, removals = 0) => {
  const removalText =
    removals > 0
      ? ` Great job removing ${removals} plant${removals > 1 ? "s" : ""} today!`
      : "";
  switch (level) {
    case "high":
...
const calculateEnvironmentalImpact = (activeInvasives, todayInvasives) => {
  ...
  const impacts = {
    ecosystemArea: (
      activeInvasives * baseImpactPerSpecies.ecosystemDegradation
    ).toFixed(2),
    waterDaily: Math.round(
      activeInvasives * baseImpactPerSpecies.waterConsumption
    ),
    soilArea: Math.round(
      activeInvasives *
        Math.PI *
        Math.pow(baseImpactPerSpecies.soilContamination, 2)
    ),
    nativeSpeciesDisplaced: Math.round(
      activeInvasives * baseImpactPerSpecies.biodiversityLoss
    ),
    carbonLoss: Math.round(
      activeInvasives * baseImpactPerSpecies.carbonReduction
    ),
  };
  ...
  const dailyTrend =
    todayInvasives > 3
      ...
      : todayInvasives > 1
      ? "Steady environmental pressure"
      : todayInvasives === 1
      ? "New environmental threat detected"
  ...
  return {
    ...
    riskScore: Math.min(100, activeInvasives * 4.2),
  };
};
...
export const generateInvasiveAnalytics = (sightings) => {
  ...
  const todayInvasives = sightings.filter(
    (s) =>
      new Date(s.createdAt).toDateString() === today.toDateString() &&
      isSightingInvasive(s) &&
      !s.isRemoved
  );
  const yesterdayInvasives = sightings.filter(
    (s) =>
      new Date(s.createdAt).toDateString() === yesterday.toDateString() &&
      isSightingInvasive(s) &&
      !s.isRemoved
  );
  const todayRemovals = sightings.filter(
    (s) =>
      s.removedAt &&
      new Date(s.removedAt).toDateString() === today.toDateString()
  );
  const yesterdayRemovals = sightings.filter(
    (s) =>
      s.removedAt &&
      new Date(s.removedAt).toDateString() === yesterday.toDateString()
  );

  const threatLevel =
    todayInvasives.length >= 5
      ? "high"
      : todayInvasives.length >= 2
      ? "medium"
      : "low";
  ...
  if (todayInvasives.length > 0)
    immediateActions.push(
      `Inspect ${todayInvasives.length} new invasive detection${
        todayInvasives.length > 1 ? "s" : ""
      }`
    );
  if (threatLevel === "high") {
  ...
  const hotspotCount = identifyHotspots(
    sightings.filter(isSightingInvasive)
  ).length;
  const recentInvasives = sightings.filter(
    (s) => new Date(s.createdAt) >= weekAgo && isSightingInvasive(s)
  );
  const spreadRate = recentInvasives.length > 1 ? Math.random() * 50 + 10 : 0;
  const activeInvasives = sightings.filter(
    (s) => isSightingInvasive(s) && !s.isRemoved
  ).length;
  ...
  return {
    ...
    todayRecommendation: getThreatRecommendation(
      threatLevel,
      todayInvasives.length,
      todayRemovals.length
    ),
    immediateActions,
    newInvasivesToday: todayInvasives.length,
    invasiveTrend: getInvasiveTrend(
      todayInvasives.length,
      yesterdayInvasives.length
    ),
    hotspotCount,
    spreadRate,
    ...
    removalTrend: getRemovalTrend(
      todayRemovals.length,
      yesterdayRemovals.length
    ),
...
export const getThreatUrgency = (sighting) => {
  ...
  if (riskLevel.toLowerCase().includes("high")) {
    ...
    return confidence > 0.8
      ? "Monitor closely - Confirmed invasive"
      : "Verify identification - Possible invasive";
  }
...
export const calculateAvgRisk = (sightings) => {
  const riskLevels = sightings.map(
    (s) => s.analysis?.llm?.details?.risk_level || "Medium"
  );
  const highCount = riskLevels.filter((r) =>
    r.toLowerCase().includes("high")
  ).length;
  const mediumCount = riskLevels.filter((r) =>
    r.toLowerCase().includes("medium")
  ).length;
  if (highCount > sightings.length / 2) return "High";
  if (mediumCount > sightings.length / 3) return "Medium";
  return "Low";
};
export const calculateWeeklySpread = (sightings) =>
  Math.floor(sightings.length * 2.5 + Math.random() * 10);
...
export const getSeasonalFactors = (month) => {
  const factors = {
    ...
    "07": "Peak biomass",
    "08": "Seed dispersal",
    "09": "Fall establishment",
    10: "Final growth push",
    11: "Dormancy prep",
    12: "Winter planning",
  };
...
export const getClimateCorrelation = () => {
  const correlations = [
    "Warmer than average - Increased activity",
    "Higher rainfall - Enhanced germination",
    "Drought - Stress on native species",
    "Extreme weather - Increased establishment",
...
export const generatePredictiveInsights = () =>
  "Based on current trends, expect 15-25% increase in invasive species next year";
...
export const generateRiskAlerts = (sightings) => {
  ...
  if (highRiskInvasives.length > 0) {
    const recentInvasive = highRiskInvasives.filter(
      (s) => new Date() - new Date(s.createdAt) < 7 * 24 * 60 * 60 * 1000
    );
  ...
  const recentSightings = activeSightings.filter(
    (s) => new Date() - new Date(s.createdAt) < 7 * 24 * 60 * 60 * 1000
  );
  if (recentSightings.length < 5) {
    alerts.info.push({
      ...
      description:
        "Detection activity is below normal levels. Consider increasing monitoring.",
      timestamp: new Date().toISOString(),
...
export const generateRecommendations = (sightings) => {
  ...
  recommendations.push({
    ...
    description:
      "Expand citizen science participation to increase detection coverage and early warning capabilities.",
    priority: "Medium",
    impact: "Medium",
  });
