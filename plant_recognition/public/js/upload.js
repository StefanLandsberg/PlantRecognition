...
function validateFile(file) {
  ...
  if (file.size > MAX_FILE_SIZE) {
    errors.push(
      `File size (${formatFileSize(file.size)}) exceeds the ${formatFileSize(
        MAX_FILE_SIZE
      )} limit.`
    );
  }
...
export function pickFile(inputId, cb) {
  ...
  input.onchange = async () => {
    ...
    if (window.showModal && f.size > 5 * 1024 * 1024) {
      // Show info for files > 5MB
      const proceed = await window.showModal.confirm(
        "Large File Upload",
        `You're uploading a ${formatFileSize(
          f.size
        )} file. This may take a moment to process.`,
        {
