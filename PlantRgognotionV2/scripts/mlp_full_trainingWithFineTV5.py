...
MODEL_NAMES = ["tf_efficientnetv2_m_in21ft1k", "convnext_base_in22ft1k"] # Dual model for feature extraction
...
# --- Feature Extractor Fine-Tuning Config ---
FE_DO_FINETUNE = True  # Set to True to enable fine-tuning the feature extractor
FE_FINETUNE_EPOCHS = 1 if not DEBUG_MODE else 1
FE_FINETUNE_LR = 1e-5
FE_FINETUNE_WEIGHT_DECAY = 1e-4
FE_FINETUNE_BATCH_SIZE = 4 if not DEBUG_MODE else 2 # Keep this very small for VRAM
FE_FINETUNE_GRAD_ACCUM_STEPS = 8 # Accumulate gradients (effective_batch_size = BATCH_SIZE * GRAD_ACCUM_STEPS)
FE_FINETUNE_OPTIMIZER_TYPE = 'AdamW' # AdamW, SGD, AdamP
FE_FINETUNE_SCHEDULER_TYPE = 'CosineAnnealingLR' # e.g., CosineAnnealingLR, ReduceLROnPlateau, None
FE_FINETUNE_EARLY_STOPPING_PATIENCE = 3
FE_FINETUNE_SWA_START_EPOCH_FACTOR = 0.70
FE_FINETUNE_USE_SWA = False # SWA for FE can be memory intensive, start with False
FE_FINETUNE_USE_EMA = False # EMA for FE, start with False
FE_FINETUNE_EMA_DECAY = 0.999
FE_FINETUNE_USE_SAM = False # SAM for FE can be very memory intensive, start with False
FE_FINETUNE_SAM_RHO = 0.05
FE_FINETUNE_SAM_ADAPTIVE = True
FE_USE_MIXED_PRECISION = True if DEVICE.type == 'cuda' else False
FE_FINETUNE_FREEZE_BACKBONE_LAYERS = 2 # Number of initial blocks/stages to freeze in each backbone. 0 means train all.
FE_FINETUNED_CHECKPOINT_BASE_DIR = os.path.join(BASE_CHECKPOINT_DIR, "fe_finetuned")
FE_FINETUNED_MODEL_SAVE_PATH = os.path.join(FE_FINETUNED_CHECKPOINT_BASE_DIR, "finetuned_feature_extractor_best.pth")
FE_FINETUNED_LATEST_CHECKPOINT_PATH = os.path.join(FE_FINETUNED_CHECKPOINT_BASE_DIR, "finetuned_feature_extractor_latest.pth.tar")

os.makedirs(FE_FINETUNED_CHECKPOINT_BASE_DIR, exist_ok=True)
...
class PlantDataset(Dataset): # Used for Feature Extraction
    def __init__(self, dataframe, image_dir, transform=None, label_encoder_instance=None, include_paths=False, image_size=None):
        ...
        # self.image_data = [] # Will be replaced by self.dataframe directly
        ...
        if not all(col in self.input_df.columns for col in required_cols):
            ...
            self.dataframe = pd.DataFrame() # Ensure dataframe is initialized even on error
            return
                    ...
                    try:
                        if os.path.getsize(full_path) > 0:
                            if filename.startswith(prefix_to_strip):
                                id_part = filename[len(prefix_to_strip):] # e.g., 63808394_0.jpg or 63808394_0_aug0.jpg
                                ...
                                if len(parts) >= 1: # Should have at least the ID part
                                    ...
                                    # Clean potential extension if it's part of obs_id_from_file (e.g. if no other underscores)
                                    if '.' in obs_id_from_file: obs_id_from_file = os.path.splitext(obs_id_from_file)[0]
                        ...
                        pass # File might be inaccessible or just disappeared
        
        self.input_df['original_index'] = self.input_df.index
        
        all_image_entries = [] # Use a temporary list to build entries
        ...
        for _, row in tqdm(self.input_df.iterrows(), total=len(self.input_df), desc="Matching CSV to image files (incl. augmentations)", leave=False, disable=DEBUG_MODE):
            try:
                ...
                # Retrieve all files (originals and their augmentations) associated with this CSV ID
        ...
        self.dataframe = pd.DataFrame(all_image_entries) # Create the final DataFrame
...
class CombinedModel(nn.Module): # Used as Feature Extractor
    ...
    def freeze_backbone_layers(self, num_stages_to_freeze_each):
        if not isinstance(num_stages_to_freeze_each, list):
            num_stages_to_freeze_each = [num_stages_to_freeze_each] * len(self.backbones)

        for i, backbone in enumerate(self.backbones):
            num_to_freeze = num_stages_to_freeze_each[i]
            if num_to_freeze <= 0:
                print(f"{TermColors.INFO}Backbone {self.model_names[i]}: No layers frozen (num_to_freeze={num_to_freeze}).{TermColors.ENDC}")
                continue
            frozen_count = 0
            if hasattr(backbone, 'conv_stem'):
                if num_to_freeze > frozen_count:
                    for param in backbone.conv_stem.parameters(): param.requires_grad = False
                    frozen_count +=1 # Counts as one "stage"
            elif hasattr(backbone, 'stem'):
                 if num_to_freeze > frozen_count:
                    for param in backbone.stem.parameters(): param.requires_grad = False
                    frozen_count +=1
            target_attr = None
            if hasattr(backbone, 'blocks'): # EfficientNet like
                target_attr = backbone.blocks
            elif hasattr(backbone, 'stages'): # ConvNeXt like
                target_attr = backbone.stages
            
            if target_attr:
                for stage_idx, stage_module in enumerate(target_attr):
                    if frozen_count < num_to_freeze:
            ...
            for param in self.embedding_layer.parameters(): param.requires_grad = True
            for param in self.metric_fc.parameters(): param.requires_grad = True

            print(f"{TermColors.INFO}Backbone {self.model_names[i]}: Attempted to freeze {num_to_freeze} stages. Actual frozen stages/parts: {frozen_count}.{TermColors.ENDC}")
...
def build_feature_extractor_model(num_classes_fe, for_finetuning=False): # Added for_finetuning flag
    # When fine-tuning, metric_learning is 'None' because we use the standard Linear head for classification.
    # Dropout for FE fine-tuning could be a small value, e.g., 0.1, if desired, but 0.0 is fine to start.
    current_dropout_rate = 0.1 if for_finetuning else 0.0 # Example: use small dropout for FE fine-tuning
    
    model = CombinedModel(MODEL_NAMES, num_classes_fe, PRETRAINED, GLOBAL_POOLING, 
                          current_dropout_rate, EMBEDDING_SIZE, DROP_PATH_RATE, 
                          ARCFACE_S, ARCFACE_M, metric_learning='None') # metric_learning='None' for classification head

    if for_finetuning and FE_FINETUNE_FREEZE_BACKBONE_LAYERS > 0:
        model.freeze_backbone_layers(FE_FINETUNE_FREEZE_BACKBONE_LAYERS)
    return model
...
def get_optimizer_fe(model): # For Feature Extractor Fine-tuning
    params = [p for p in model.parameters() if p.requires_grad]
    if not params: raise ValueError("No trainable parameters in FE model for fine-tuning")
    
    lr = FE_FINETUNE_LR
    wd = FE_FINETUNE_WEIGHT_DECAY
    optim_type = FE_FINETUNE_OPTIMIZER_TYPE
    use_sam = FE_FINETUNE_USE_SAM
    sam_rho_val = FE_FINETUNE_SAM_RHO
    sam_adaptive_val = FE_FINETUNE_SAM_ADAPTIVE
    ...
    if use_sam and SAM_AVAILABLE:
        ...
        else: base_opt_fn = partial(optim.AdamW, lr=lr, weight_decay=wd); print(f"{TermColors.YELLOW}WARN: SAM base for FE defaulting to AdamW.{TermColors.ENDC}")
        print(f"{TermColors.BLUE}INFO: Using SAM for FE (rho={sam_rho_val}, adaptive={sam_adaptive_val}). Base: {optim_type}{TermColors.ENDC}")
        return SAM(params, base_opt_fn, rho=sam_rho_val, adaptive=sam_adaptive_val)
    else:
        if optim_type == 'AdamP' and ADAMP_AVAILABLE: return AdamP(params, lr=lr, weight_decay=wd, betas=(0.9, 0.999), nesterov=True)
        if optim_type == 'AdamW': return optim.AdamW(params, lr=lr, weight_decay=wd)
        if optim_type == 'SGD': return optim.SGD(params, lr=lr, weight_decay=wd, momentum=0.9, nesterov=True)
        print(f"{TermColors.YELLOW}WARN: FE Optimizer '{optim_type}' not supported or not avail. Defaulting to AdamW.{TermColors.ENDC}")
        return optim.AdamW(params, lr=lr, weight_decay=wd)

def get_scheduler_fe(optimizer, total_epochs_for_sched): # For Feature Extractor Fine-tuning
    opt_for_scheduler = optimizer.base_optimizer if hasattr(optimizer, 'base_optimizer') else optimizer
    sched_type = FE_FINETUNE_SCHEDULER_TYPE
    lr = FE_FINETUNE_LR
    
    if sched_type == 'CosineAnnealingLR':
        return CosineAnnealingLR(opt_for_scheduler, T_max=total_epochs_for_sched, eta_min=lr * 0.01)
    elif sched_type == 'ReduceLROnPlateau':
        return ReduceLROnPlateau(opt_for_scheduler, mode=CHECKPOINT_MODE, factor=0.2, patience=max(1, FE_FINETUNE_EARLY_STOPPING_PATIENCE // 2), min_lr=lr * 0.001, monitor=CHECKPOINT_MONITOR)
    elif sched_type == 'CosineWarmRestarts': # Added for completeness, T_0 should be managed
         return CosineAnnealingWarmRestarts(opt_for_scheduler, T_0=max(1, total_epochs_for_sched // 2), T_mult=1, eta_min=lr * 0.01)
...
# --- Training & Validation Loops for Feature Extractor Fine-Tuning ---
def train_one_epoch_fe(model, dataloader, criterion, optimizer, scaler, scheduler, global_epoch, device, writer, ema_model_fe):
    model.train()
    running_loss = 0.0
    total_samples = 0
    all_preds, all_labels = [], []
    is_sam_active = hasattr(optimizer, 'base_optimizer') and SAM_AVAILABLE and FE_FINETUNE_USE_SAM
    grad_accum_steps = FE_FINETUNE_GRAD_ACCUM_STEPS

    pbar_desc = f"FE Tune E{global_epoch+1} Tr"
    progress_bar = tqdm(dataloader, desc=pbar_desc, leave=True, bar_format='{l_bar}{bar:30}{r_bar}{bar:-30b}')
    
    # optimizer.zero_grad() # Moved inside the accumulation step logic for clarity, 
                            # effectively done after each optimizer update.
                            # The first zero_grad for the epoch happens before the loop by the caller or implicitly.
                            # For grad_accum > 1, zero_grad must happen after scaler.step/optimizer.second_step

    for batch_idx, (inputs, labels_orig, _, _) in enumerate(progress_bar): # (images, labels, paths, original_indices)
        if check_keyboard_stop(): break
        inputs, labels_orig = inputs.to(device), labels_orig.to(device)
        batch_size = inputs.size(0)

        def get_active_outputs_fe(raw_outputs): # LogitAdjust for FE if ever used, though typically not
            if IMBALANCE_STRATEGY == 'LogitAdjust' and CLASS_PRIORS is not None: # Assuming CLASS_PRIORS is global
                return raw_outputs + (LOGIT_ADJUSTMENT_TAU * torch.log(CLASS_PRIORS + 1e-12)).unsqueeze(0)
            return raw_outputs
        
        loss_this_minibatch_for_metric = None # Store the loss of the current minibatch for display
        outputs_for_metric = None

        with torch.amp.autocast('cuda', enabled=(FE_USE_MIXED_PRECISION and device.type == 'cuda')):
            if is_sam_active:
                # SAM First Pass (ascent step to find perturbation e(w))
                # Calculate loss L_S(w) to find gradient for perturbation
                outputs1_raw = model(inputs, labels=None, return_embedding=False)
                outputs1_active = get_active_outputs_fe(outputs1_raw)
                loss1 = criterion(outputs1_active, labels_orig) # This loss is for perturbation, not divided by grad_accum_steps

                if torch.isnan(loss1) or torch.isinf(loss1):
                    print(f"{TermColors.RED}NaN/Inf loss in FE SAM perturbation (L_S(w)). Skipping batch {batch_idx}.{TermColors.ENDC}")
                    continue 
                
                scaler.scale(loss1).backward() # Compute gradients dL_S(w)/dw
                optimizer.first_step(zero_grad=True) # Perturb weights: w <- w + e(w), and zero base_optimizer grads. Model weights are now perturbed.

                outputs2_raw = model(inputs, labels=None, return_embedding=False) # Forward pass with perturbed weights
                outputs2_active = get_active_outputs_fe(outputs2_raw)
                loss2 = criterion(outputs2_active, labels_orig) # This is L_S(w_perturbed)
                
                loss_this_minibatch_for_metric = loss2.item() # For display
                outputs_for_metric = outputs2_active

                if torch.isnan(loss2) or torch.isinf(loss2):
                    print(f"{TermColors.RED}NaN/Inf loss in FE SAM descent (L_S(w_perturbed)). Skipping gradient accumulation for batch {batch_idx}.{TermColors.ENDC}")
                    continue # Skip accumulating gradients for this failed batch
                
                # Scale loss for accumulation and backward pass
                loss_to_accumulate = loss2 / grad_accum_steps
                scaler.scale(loss_to_accumulate).backward() # Accumulate gradients dL_S(w_perturbed)/dw

            else: # Standard non-SAM training for FE
                outputs_raw = model(inputs, labels=None, return_embedding=False) # FE model uses its linear head
                outputs_active = get_active_outputs_fe(outputs_raw)
                current_loss = criterion(outputs_active, labels_orig)
                
                loss_this_minibatch_for_metric = current_loss.item() # For display
                outputs_for_metric = outputs_active

                if torch.isnan(current_loss) or torch.isinf(current_loss):
                    print(f"{TermColors.RED}NaN/Inf loss in FE training (non-SAM). Skipping gradient accumulation for batch {batch_idx}.{TermColors.ENDC}")
                    continue # Skip accumulating gradients

                loss_to_accumulate = current_loss / grad_accum_steps # Normalize loss for accumulation
                scaler.scale(loss_to_accumulate).backward() # Accumulate gradients

        # End of AMP context

        # Perform optimizer step after accumulating gradients for `grad_accum_steps`
        if (batch_idx + 1) % grad_accum_steps == 0 or (batch_idx + 1) == len(dataloader):
            if is_sam_active:
                scaler.unscale_(optimizer.base_optimizer) # Unscale accumulated gradients in the base optimizer
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
                optimizer.second_step(zero_grad=True) # Perform update using accumulated grads and zero base_optimizer grads
            else: # Non-SAM
            ...
            scaler.update()
            optimizer.zero_grad() # Crucial: Zero gradients for the next accumulation cycle for all optimizers
                                  # For SAM, second_step(zero_grad=True) handles the base_optimizer.
                                  # This ensures the SAM optimizer wrapper itself (if it had any state) or standard optimizer is cleared.
            
            if FE_FINETUNE_USE_EMA and ema_model_fe:
                ema_model_fe.update_parameters(model)

        # Logging and metrics, using loss_this_minibatch_for_metric (unnormalized)
        if loss_this_minibatch_for_metric is not None and not (np.isnan(loss_this_minibatch_for_metric) or np.isinf(loss_this_minibatch_for_metric)):
            running_loss += loss_this_minibatch_for_metric * batch_size # Use un-normalized loss for epoch loss calculation
            total_samples += batch_size
            if outputs_for_metric is not None:
                preds = torch.argmax(outputs_for_metric, dim=1)
                all_preds.append(preds.detach().cpu())
                all_labels.append(labels_orig.detach().cpu())

            if batch_idx % (20 * grad_accum_steps) == 0 or batch_idx == len(dataloader) -1 :
                # Display loss of the current minibatch (not the accumulated/normalized one)
                display_loss = loss_this_minibatch_for_metric 
                if total_samples > 0 and all_preds: # Make sure all_preds is not empty
                    current_acc_display = (torch.cat(all_preds) == torch.cat(all_labels)).sum().item() / total_samples
                    current_lr_val_display = optimizer.param_groups[0]['lr']
                    if hasattr(optimizer, 'base_optimizer'): current_lr_val_display = optimizer.base_optimizer.param_groups[0]['lr']
                    progress_bar.set_postfix(loss=f"{display_loss:.3f}", acc=f"{current_acc_display:.3f}", lr=f"{current_lr_val_display:.1E}")
                elif display_loss is not None : # Check display_loss before using
                     progress_bar.set_postfix(loss=f"{display_loss:.3f}")
    
    if stop_requested: return None, None
    epoch_loss = running_loss / total_samples if total_samples > 0 else float('nan')
    epoch_acc = (torch.cat(all_preds) == torch.cat(all_labels)).sum().item() / total_samples if total_samples > 0 and all_preds and all_labels else 0.0
    
    # SWA scheduler step is handled in the main FE fine-tuning loop
    if scheduler and not isinstance(scheduler, ReduceLROnPlateau) and not (FE_FINETUNE_USE_SWA and global_epoch >= int(FE_FINETUNE_EPOCHS * FE_FINETUNE_SWA_START_EPOCH_FACTOR)):
        scheduler.step()

    if writer:
        if not np.isnan(epoch_loss): writer.add_scalar(f'FE_FineTune/Loss/train', epoch_loss, global_epoch)
        writer.add_scalar(f'FE_FineTune/Accuracy/train', epoch_acc, global_epoch)
        current_lr_writer_fe = optimizer.param_groups[0]['lr']
        if hasattr(optimizer, 'base_optimizer'): current_lr_writer_fe = optimizer.base_optimizer.param_groups[0]['lr']
        writer.add_scalar(f'FE_FineTune/LearningRate', current_lr_writer_fe, global_epoch)
    return epoch_loss, epoch_acc

def validate_one_epoch_fe(model, dataloader, criterion, device, global_epoch, writer, scheduler=None, swa_model_fe=None, ema_model_fe=None):
    model.eval()
    if swa_model_fe: swa_model_fe.eval()
    if ema_model_fe: ema_model_fe.eval()
    
    results_fe = {}
    models_to_eval_fe = {'base_fe': model}
    if FE_FINETUNE_USE_SWA and swa_model_fe: models_to_eval_fe['swa_fe'] = swa_model_fe
    if FE_FINETUNE_USE_EMA and ema_model_fe: models_to_eval_fe['ema_fe'] = ema_model_fe

    with torch.no_grad():
        for model_key, current_model_fe in models_to_eval_fe.items():
            current_model_fe.eval()
            model_running_loss = 0.0
            model_total_samples = 0
            model_all_preds, model_all_labels = [], []
            
            pbar_desc = f"FE Tune Val E{global_epoch+1} ({model_key})"
            progress_bar = tqdm(dataloader, desc=pbar_desc, leave=False)

            for (inputs, labels, _, _) in progress_bar: # (images, labels, paths, original_indices)
                inputs, labels = inputs.to(device), labels.to(device)
                batch_size = inputs.size(0)
                
                with torch.amp.autocast('cuda', enabled=(FE_USE_MIXED_PRECISION and device.type == 'cuda')):
                    outputs = current_model_fe(inputs, labels=None, return_embedding=False) # FE model uses its linear head
                    adj_outputs = outputs # Placeholder for LogitAdjust if ever used for FE
                    loss = criterion(adj_outputs, labels)
                
                if not torch.isnan(loss):
                    model_running_loss += loss.item() * batch_size
                    model_total_samples += batch_size
                    preds = torch.argmax(adj_outputs, dim=1)
                    model_all_preds.append(preds.detach().cpu())
                    model_all_labels.append(labels.detach().cpu())
                else:
                    print(f"{TermColors.RED}Warn: NaN/Inf FE validation loss. Skipping batch.{TermColors.ENDC}")
                progress_bar.set_postfix(loss=f"{loss.item():.4f}")
                if stop_requested: return None, None
            
            epoch_loss = model_running_loss / model_total_samples if model_total_samples > 0 else float('inf')
            ...
            results_fe[model_key] = (epoch_loss, epoch_acc)

            if writer:
                writer.add_scalar(f'FE_FineTune/Loss/val_{model_key}', epoch_loss, global_epoch)
                writer.add_scalar(f'FE_FineTune/Accuracy/val_{model_key}', epoch_acc, global_epoch)
                
    base_loss, base_acc = results_fe.get('base_fe', (float('inf'), 0.0))
    
    if scheduler and isinstance(scheduler, ReduceLROnPlateau):
        metric_to_monitor = base_loss if CHECKPOINT_MONITOR == 'val_loss' else base_acc # Assuming CHECKPOINT_MONITOR is global
...
def extract_all_features(df_full_fe, num_classes_fe, label_encoder_fe):
    global NUM_CLASSES, label_encoder, class_names 
    ...
    # Build model without freezing for feature extraction pass
    feature_extractor = build_feature_extractor_model(NUM_CLASSES, for_finetuning=False) 
    
    # Try to load the self-finetuned model first if FE_DO_FINETUNE was True
    model_loaded_path = None
    if FE_DO_FINETUNE and os.path.exists(FE_FINETUNED_MODEL_SAVE_PATH):
        print(f"{TermColors.CYAN}Loading self-finetuned FE model: {FE_FINETUNED_MODEL_SAVE_PATH}{TermColors.ENDC}")
        try:
            # The saved model is a state_dict
            state_dict = torch.load(FE_FINETUNED_MODEL_SAVE_PATH, map_location=DEVICE)
            # Adjust keys if saved with DataParallel or torch.compile
            new_state_dict = {}
            is_compiled_ckpt = any(k.startswith('_orig_mod.') for k in state_dict.keys())
            is_module_ckpt = any(k.startswith('module.') for k in state_dict.keys())

            for k, v in state_dict.items():
                name = k
                if is_module_ckpt: name = name.replace('module.', '')
                if is_compiled_ckpt: name = name.replace('_orig_mod.', '')
                new_state_dict[name] = v
            feature_extractor.load_state_dict(new_state_dict, strict=True) # Be strict for final FE model
            model_loaded_path = FE_FINETUNED_MODEL_SAVE_PATH
        except Exception as e: 
            print(f"{TermColors.RED}Self-finetuned FE Load Fail: {e}. Check path/keys.{TermColors.ENDC}")
            print(f"{TermColors.YELLOW}Falling back to FEATURE_EXTRACTOR_CHECKPOINT_PATH or pretrained.{TermColors.ENDC}")
            model_loaded_path = None # Reset

    if not model_loaded_path and FEATURE_EXTRACTOR_CHECKPOINT_PATH and os.path.exists(FEATURE_EXTRACTOR_CHECKPOINT_PATH):
        print(f"{TermColors.CYAN}Loading pre-specified FE checkpoint: {FEATURE_EXTRACTOR_CHECKPOINT_PATH}{TermColors.ENDC}")
        try:
            ckpt = torch.load(FEATURE_EXTRACTOR_CHECKPOINT_PATH, map_location=DEVICE)
            # Assuming this checkpoint is also a state_dict or has 'state_dict' key
            state_dict_to_load = ckpt['state_dict'] if 'state_dict' in ckpt else ckpt
            
            new_state_dict = {}
            is_compiled_ckpt = any(k.startswith('_orig_mod.') for k in state_dict_to_load.keys())
            is_module_ckpt = any(k.startswith('module.') for k in state_dict_to_load.keys())
            for k, v in state_dict_to_load.items():
                name = k
                if is_module_ckpt: name = name.replace('module.', '')
                if is_compiled_ckpt: name = name.replace('_orig_mod.', '')
                new_state_dict[name] = v
            feature_extractor.load_state_dict(new_state_dict, strict=False) # Allow strict=False for general checkpoints
            model_loaded_path = FEATURE_EXTRACTOR_CHECKPOINT_PATH
        except Exception as e: 
            print(f"{TermColors.RED}Pre-specified FE Ckpt Load Fail: {e}. Using default pretrained.{TermColors.ENDC}")
            model_loaded_path = None # Reset

    if not model_loaded_path:
        print(f"{TermColors.INFO}Using default timm pretrained weights for feature extractor.{TermColors.ENDC}")

    feature_extractor = feature_extractor.to(DEVICE).eval()
    if USE_TORCH_COMPILE and hasattr(torch, 'compile'): # Compile for inference if enabled
        try: feature_extractor = torch.compile(feature_extractor, mode='reduce-overhead')
        except: print(f"{TermColors.RED}torch.compile for FE failed.{TermColors.ENDC}")
...
def main():
    ...
    try: # Data Loading and Preprocessing
        ...
        df_full = df_full[required_cols].dropna().astype({'id': str}); df_full.rename(columns={'scientific_name': 'scientificName'}, inplace=True)
        
        min_samples_in_csv_for_stratification = 2 
        print(f"{TermColors.INFO}Filtering CSV for FE split: Keeping classes with at least {min_samples_in_csv_for_stratification} observations in the CSV to ensure stratifiability of df_full.{TermColors.ENDC}")
        class_counts = df_full['scientificName'].value_counts()
        valid_classes = class_counts[class_counts >= min_samples_in_csv_for_stratification].index
        original_row_count = len(df_full)
        df_full = df_full[df_full['scientificName'].isin(valid_classes)].reset_index(drop=True)
        
        if len(df_full) < original_row_count:
            print(f"{TermColors.YELLOW}WARN: Dropped {original_row_count - len(df_full)} rows from CSV that belonged to classes with < {min_samples_in_csv_for_stratification} CSV observations.{TermColors.ENDC}")

        if len(df_full) == 0: 
            print(f"{TermColors.RED}Dataframe empty after filtering for >= {min_samples_in_csv_for_stratification} samples per class in CSV. Exiting.{TermColors.ENDC}")
            sys.exit(1)
    ...
    # --- Feature Extractor Fine-Tuning (New Stage) ---
    if FE_DO_FINETUNE:
        print(f"\n{TermColors.HEADER}--- STEP 1.5: Feature Extractor Fine-Tuning ---{TermColors.ENDC}")
        if os.path.exists(FE_FINETUNED_MODEL_SAVE_PATH) and not DEBUG_MODE: # In debug, always re-tune
            print(f"{TermColors.INFO}Found existing fine-tuned FE model: {FE_FINETUNED_MODEL_SAVE_PATH}. Skipping FE fine-tuning.{TermColors.ENDC}")
        else:
            try:
                fe_model = build_feature_extractor_model(NUM_CLASSES, for_finetuning=True).to(DEVICE)
                
                # DataLoaders for FE fine-tuning (using image data)
                min_samples_per_class_fe_split = 2 
                value_counts_fe = df_full['label'].value_counts()
                labels_to_stratify_fe = df_full['label']

                if any(value_counts_fe < min_samples_per_class_fe_split):
                    print(f"{TermColors.YELLOW}WARN: Some classes in df_full (from CSV) have < {min_samples_per_class_fe_split} samples. Stratification for FE tuning might be imperfect or fail.{TermColors.ENDC}")
                
                try:
                    df_fe_train, df_fe_val = train_test_split(
                        df_full, 
                        test_size=0.15, 
                        random_state=SEED + 200, 
                        stratify=labels_to_stratify_fe
                    )
                    print(f"{TermColors.INFO}FE fine-tuning data split using stratification.{TermColors.ENDC}")
                except ValueError as e_split:
                    print(f"{TermColors.YELLOW}WARN: Stratified split for FE failed (Error: {e_split}). Attempting non-stratified split.{TermColors.ENDC}")
                    df_fe_train, df_fe_val = train_test_split(
                        df_full, 
                        test_size=0.15, 
                        random_state=SEED + 200, 
                        stratify=None # Fallback to non-stratified
                    )
                    print(f"{TermColors.INFO}FE fine-tuning data split using non-stratified fallback.{TermColors.ENDC}")
                
                fe_train_transform = get_transforms(image_size=FEATURE_EXTRACTION_IMAGE_SIZE_CONFIG, for_feature_extraction=True) # Use basic FE transforms
                fe_val_transform = get_transforms(image_size=FEATURE_EXTRACTION_IMAGE_SIZE_CONFIG, for_feature_extraction=True)

                # Set include_paths to True for FE fine-tuning datasets
                fe_train_ds = PlantDataset(df_fe_train, IMAGE_DIR, fe_train_transform, label_encoder, True, FEATURE_EXTRACTION_IMAGE_SIZE_CONFIG)
                fe_val_ds = PlantDataset(df_fe_val, IMAGE_DIR, fe_val_transform, label_encoder, True, FEATURE_EXTRACTION_IMAGE_SIZE_CONFIG)
                
                if len(fe_train_ds) == 0 or len(fe_val_ds) == 0:
                    print(f"{TermColors.RED}FE fine-tuning dataset split resulted in empty train/val set. Skipping FE fine-tuning.{TermColors.ENDC}")
                else:
                    fe_train_loader = DataLoader(fe_train_ds, FE_FINETUNE_BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, pin_memory=True, drop_last=True)
                    fe_val_loader = DataLoader(fe_val_ds, FE_FINETUNE_BATCH_SIZE * 2, shuffle=False, num_workers=NUM_WORKERS, pin_memory=True)

                    fe_optimizer = get_optimizer_fe(fe_model)
                    fe_scheduler = get_scheduler_fe(fe_optimizer, total_epochs_for_sched=FE_FINETUNE_EPOCHS)
                    fe_scaler = torch.amp.GradScaler('cuda', enabled=(FE_USE_MIXED_PRECISION and DEVICE.type == 'cuda'))
                    # Use standard CE loss for FE fine-tuning, label smoothing can be 0 or small
                    fe_criterion = get_criterion(CLASS_WEIGHTS, current_label_smoothing=0.0, is_mlp_criterion=False)  
                    
                    fe_writer = SummaryWriter(log_dir=os.path.join(BASE_LOG_DIR, "fe_finetune"))
                    
                    best_fe_val_acc = 0.0
                    epochs_no_improve_fe = 0
                    start_epoch_fe = 0

                    # Checkpoint loading for FE fine-tuning
                    if os.path.exists(FE_FINETUNED_LATEST_CHECKPOINT_PATH):
                        print(f"{TermColors.CYAN}Loading latest FE fine-tuning checkpoint...{TermColors.ENDC}")
                        # Simplified load_checkpoint call for FE state_dict only for now
                        try:
                            ckpt_fe = torch.load(FE_FINETUNED_LATEST_CHECKPOINT_PATH, map_location=DEVICE)
                            fe_model.load_state_dict(ckpt_fe['state_dict'])
                            fe_optimizer.load_state_dict(ckpt_fe['optimizer'])
                            if fe_scheduler and ckpt_fe.get('scheduler'): fe_scheduler.load_state_dict(ckpt_fe['scheduler'])
                            if fe_scaler and ckpt_fe.get('scaler'): fe_scaler.load_state_dict(ckpt_fe['scaler'])
                            start_epoch_fe = ckpt_fe.get('global_epoch', 0)
                            best_fe_val_acc = ckpt_fe.get('best_metric', 0.0)
                            print(f"{TermColors.GREEN}Resuming FE fine-tuning from epoch {start_epoch_fe}, best acc: {best_fe_val_acc:.4f}{TermColors.ENDC}")
                        except Exception as e_fe_load:
                            print(f"{TermColors.RED}Error loading FE fine-tune checkpoint: {e_fe_load}. Starting fresh.{TermColors.ENDC}")
                            start_epoch_fe = 0; best_fe_val_acc = 0.0
                    
                    # SWA/EMA for FE (optional, can be complex with checkpointing)
                    fe_swa_model = None; fe_ema_model = None # Simplified for now

                    for epoch_fe in range(start_epoch_fe, FE_FINETUNE_EPOCHS):
                        if stop_requested: break
                        print(f"\n{TermColors.CYAN} FE Fine-Tune Epoch {epoch_fe+1}/{FE_FINETUNE_EPOCHS} {TermColors.ENDC}")
                        
                        train_loss_fe, train_acc_fe = train_one_epoch_fe(
                            fe_model, fe_train_loader, fe_criterion, fe_optimizer, fe_scaler, 
                            fe_scheduler if not FE_FINETUNE_USE_SWA else None, # SWA has its own scheduler logic
                            epoch_fe, DEVICE, fe_writer, fe_ema_model
                        )
                        if train_loss_fe is None: stop_requested = True; break # Handle interruption

                        val_loss_fe, val_acc_fe = validate_one_epoch_fe(
                            fe_model, fe_val_loader, fe_criterion, DEVICE, epoch_fe, fe_writer, 
                            fe_scheduler, fe_swa_model, fe_ema_model
                        )
                        if val_loss_fe is None: stop_requested = True; break # Handle interruption

                        print(f"FE Tune Ep {epoch_fe+1}: Tr L={train_loss_fe:.4f} A={train_acc_fe:.4f} | Val L={val_loss_fe:.4f} A={val_acc_fe:.4f} (Best Val Acc: {best_fe_val_acc:.4f})")

                        if val_acc_fe > best_fe_val_acc:
                            best_fe_val_acc = val_acc_fe
                            epochs_no_improve_fe = 0
                            print(f"{TermColors.OKGREEN}New best FE val_acc: {best_fe_val_acc:.4f}. Saving model to {FE_FINETUNED_MODEL_SAVE_PATH}{TermColors.ENDC}")
                            torch.save(fe_model.state_dict(), FE_FINETUNED_MODEL_SAVE_PATH) # Save just state_dict
                        else:
                            epochs_no_improve_fe += 1

                        # Save latest checkpoint for FE fine-tuning
                        save_checkpoint(fold="fe_tune", global_epoch=epoch_fe + 1, model=fe_model, optimizer=fe_optimizer,
                                        scheduler=fe_scheduler, scaler=fe_scaler, best_metric=best_fe_val_acc,
                                        filename=os.path.basename(FE_FINETUNED_LATEST_CHECKPOINT_PATH), is_mlp_checkpoint=False)


                        if epochs_no_improve_fe >= FE_FINETUNE_EARLY_STOPPING_PATIENCE:
                            print(f"{TermColors.WARNING}FE fine-tuning early stopping triggered at epoch {epoch_fe+1}.{TermColors.ENDC}")
                            break
                        gc.collect(); torch.cuda.empty_cache()
                    
                    if fe_writer: fe_writer.close()
                    del fe_model, fe_optimizer, fe_scheduler, fe_scaler, fe_criterion, fe_train_loader, fe_val_loader, fe_train_ds, fe_val_ds
                    gc.collect(); torch.cuda.empty_cache()

            except Exception as e_fe_tune:
                print(f"{TermColors.RED}Error during FE fine-tuning: {e_fe_tune}{TermColors.ENDC}")
                traceback.print_exc()
    ...
    if os.path.exists(STACKING_META_MODEL_PATH) and RUN_STACKING and not DEBUG_MODE and not FE_DO_FINETUNE: # Skip if FE was just tuned
    ...
    elif os.path.exists(STACKING_OOF_PREDS_PATH) and RUN_STACKING and not DEBUG_MODE and not FE_DO_FINETUNE: # Skip if FE was just tuned
    ...
    if not training_successful or (FE_DO_FINETUNE and not stop_requested): # Run MLP if not successful OR if FE tuning just happened
        print(f"{TermColors.INFO}Starting full MLP K-fold training and stacking.{TermColors.ENDC}")
        if not os.path.exists(FEATURES_NPZ_PATH) or FE_DO_FINETUNE: # Re-extract if FE was tuned
            if FE_DO_FINETUNE: print(f"{TermColors.YELLOW}Re-extracting features after FE fine-tuning...{TermColors.ENDC}")
            elif not os.path.exists(FEATURES_NPZ_PATH): print(f"{TermColors.YELLOW}Base features not found: {FEATURES_NPZ_PATH}. Extracting features...{TermColors.ENDC}")
            
            extract_all_features(df_full, NUM_CLASSES, label_encoder) # This will now use the fine-tuned FE if path exists
