...
# Configuration
DATA_DIR = r"C:\Users\stefa\Desktop\PlantRecognition\PlantRecognition\data\plant_images"
IMAGE_SIZE = (224, 224)
                ...
                def save_augmented_images(result_data):
                    ...
                    try:
                        ...
                        for i, augmented_img in enumerate(augmented_images):
                            ...
                            # Convert back to uint8 for saving - fix the conversion formula
                            # EfficientNetV2 preprocess_input uses tf.keras.applications.imagenet_utils.preprocess_input 
                            # with mode 'tf' which normalizes to [-1, 1]
                            save_img = ((augmented_img + 1.0) * 127.5).astype(np.uint8)
                            
                            # Check if we got a valid image
                            if save_img.shape[2] != 3 or save_img.min() < 0 or save_img.max() > 255:
                                print(f"Warning: Invalid image for {basename}_aug{i}_{counter}.jpg. Skipping.")
                                continue
                                
                            # Save with OpenCV - convert from RGB to BGR
...
def run_augmentation():
    ...
    # Check for CUDA/GPU support using recommended approach
    physical_devices = tf.config.list_physical_devices('GPU')
